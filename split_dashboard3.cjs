...
const globalDashboardContent = lines.slice(43, 395).join('\n'); // Everything inside "if (!activeVaultItem) { ... }" except the closing bracket
const groupDashboardContent = lines.slice(397, 757).join('\n'); // Everything after "if (!activeVaultItem)"
